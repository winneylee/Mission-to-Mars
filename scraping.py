...
def scrape_all():
    ...
    data = {
        ...
        "hemisphere_image":hemisphere_image(browser)
    }
...
def hemisphere_image(browser):
    ...
    try:
        ...
        for i in range(pics_count):
            ...
            sub_page_html = browser.html
            sub_page_soup = soup(sub_page_html, 'html.parser')
    
            img_url = sub_page_soup.select_one("div.downloads ul li a").get('href')
            # Get the full image title
            img_title = sub_page_soup.select_one("h2.title").get_text()
